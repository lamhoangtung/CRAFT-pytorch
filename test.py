...
parser.add_argument('--output_folder', default='./result/',
                    type=str, help='folder path to output images')
parser.add_argument('--debug', default=True,
                    action='store_true', help='Save visualized bounding box images')
...
if __name__ == '__main__':
    ...
    for k, image_path in enumerate(image_list):
        ...
        bboxes, polys, score_text = test_net(
            net, image, args.text_threshold, args.link_threshold, args.low_text, cuda, args.poly)

        import ipdb; ipdb.set_trace()

        # save score text
        filename, file_ext = os.path.splitext(os.path.basename(image_path))
        mask_file = args.output_folder + "/res_" + filename + '_mask.jpg'
